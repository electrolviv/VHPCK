...
class VHPCK { // Deps: u8 unsigned char, u16 - unsigned short, u32 - unsigned int32
	...
	public:
		...
		verr Encode(const u8 *src, u16 len, u8 *pResult, u16 lim) {
			...
			for(u16 i=0;i<symcount;i++) { PByte(&wr,backref[i]); u8 lenb = Path(i); while(lenb--) { PBit(&wr,1); PBit(&wr, TBit(&tr) ); DecPtr(&tr); } PBit(&wr,0); }
			 // Store Data, rev, direct path ... Save reversed path
			for(u16 i=0;i<len;i++) { u8 lenb = Path(refs[src[i]]); while(lenb--) { PBit( &wr, TBit(&tr)); DecPtr(&tr); } }
			// Store header, 0x56484831 is VHH1
			stHDR *phdr = (stHDR *)pResult; phdr->pfx = 0x31484856; phdr->leno = len; phdr->lenp = wr.ptr - pResult + (wr.msk == 0x80 ? 0 : 1) - sizeof(stHDR) ;
			return vok;
		}

		verr Decode(const u8 *src, u8 *pResult, u16 lim) {
			...
			while(1) { u8 dir = GBit(&rd); cur = !dir ? left[cur] : right[cur];
